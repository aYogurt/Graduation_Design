...
# getTxtFile() : 取标签名，且其在txt文件样本量在180-200范围（使样本均衡）
def getTxtFile(inputpath):
...
# filepath = "G:\\data_txt200"
# fatherLists = getTxtFile(filepath)
# len(fatherList)  #  104
# print(fatherLists)
# ['串通投标罪.txt', '交通肇事罪.txt', '介绍贿赂罪.txt', '以危险方法危害公共安全罪.txt', '传播淫秽物品罪.txt', '伪证罪.txt', '侵占罪.txt', '侵犯公民个人信息罪.txt', '侵犯著作权罪.txt', '保险诈骗罪.txt', '信用卡诈骗罪.txt', '假冒注册商标罪.txt', '偷越国（边）境罪.txt', '冒充军人招摇撞骗罪.txt', '刑事其他.txt', '协助组织卖淫罪.txt', '单位受贿罪.txt', '单位行贿罪.txt', '危险驾驶罪.txt', '受贿罪.txt', '合同诈骗罪.txt', '失火罪.txt', '妨害作证罪.txt', '妨害信用卡管理罪.txt', '妨害公务罪.txt', '容留他人吸毒罪.txt', '对非国家工作人员行贿罪.txt', '寻衅滋事罪.txt', '开设赌场罪.txt', '强奸罪.txt', '强迫交易罪.txt', '徇私枉法罪.txt', '扰乱无线电通讯管理秩序罪.txt', '投放危险物质罪.txt', '抢劫罪.txt', '抢夺罪.txt', '拐骗儿童罪.txt', '拒不支付劳动报酬罪.txt', '招摇撞骗罪.txt', '持有伪造的发票罪.txt', '挪用公款罪.txt', '挪用资金罪.txt', '放火罪.txt', '故意伤害罪.txt', '故意杀人罪.txt', '故意毁坏财物罪.txt', '敲诈勒索罪.txt', '污染环境罪.txt', '滥伐林木罪.txt', '滥用职权罪.txt', '爆炸罪.txt', '猥亵儿童罪.txt', '玩忽职守罪.txt', '盗伐林木罪.txt', '盗窃罪.txt', '破坏易燃易爆设备罪.txt', '破坏生产经营罪.txt', '破坏电力设备罪.txt', '破坏监管秩序罪.txt', '票据诈骗罪.txt', '私分国有资产罪.txt', '组织他人偷越国（边）境罪.txt', '组织卖淫罪.txt', '绑架罪.txt', '职务侵占罪.txt', '聚众冲击国家机关罪.txt', '聚众扰乱社会秩序罪.txt', '聚众斗殴罪.txt', '脱逃罪.txt', '虚开发票罪.txt', '虚报注册资本罪.txt', '行贿罪.txt', '诈骗罪.txt', '诬告陷害罪.txt', '贪污罪.txt', '贷款诈骗罪.txt', '赌博罪.txt', '过失以危险方法危害公共安全罪.txt', '过失致人死亡罪.txt', '过失致人重伤罪.txt', '运送他人偷越国（边）境罪.txt', '违法发放贷款罪.txt', '逃税罪.txt', '遗弃罪.txt', '重大劳动安全事故罪.txt', '重大责任事故罪.txt', '重婚罪.txt', '销售假冒注册商标的商品罪.txt', '集资诈骗罪.txt', '非国家工作人员受贿罪.txt', '非法买卖制毒物品罪.txt', '非法入侵住宅罪.txt', '非法出售发票罪.txt', '非法占用农用地罪.txt', '非法吸收公众存款罪.txt', '非法拘禁罪.txt', '非法持有毒品罪.txt', '非法捕捞水产品罪.txt', '非法狩猎罪.txt', '非法种植毒品原植物罪.txt', '非法经营罪.txt', '非法行医罪.txt', '非法进行节育手术罪.txt', '非法采矿罪.txt']
...
def getDataLabelList(inputPath):
    dataList = []
    labelList = []
    fatherLists = getTxtFile(inputPath)  # 目录 ['xx罪.txt','xx罪.txt',...]
...
# filepath = "G:\\data_txt200"
# data_list, label_list = getDataLabelList(filepath)
# print(len(data_list))
...
def getPreResult(data_list, label_list, stopWordList):
...
if __name__ == '__main__':
  ...
  # 你运行的程序，也就是你的代码块，或者函数
    ...
    getPreResult(data_list, label_list, stopWordList)
